...
Canvas::Canvas(int width, int height) ///constructorul
{
	this->width = width; ///initializam cele 2 dimensiuni
	this->height = height;
	...
	for (int i = 0; i < height; ++i) ///parcurgem fiecare rand al matricei
	{
		...
		for (int j = 0; j < width; ++j) ///parcurgem fiecare rand
			matrix[i][j] = ' '; ///initializam fiecare element cu ' '
	}
}

void Canvas::DrawCircle(int x, int y, int ray, char ch) ///(x,y) coordonatele centului, raza, caracterul cu care desenam conturul
{
...
void Canvas::Clear() // refacem matricea initiala, cu ' '
{
	for (int i = 0; i < this->height; ++i)
		for (int j = 0; j < this->width; ++j)
			matrix[i][j] = ' ';
